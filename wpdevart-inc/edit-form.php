						...
						<?php
						 if(isset($_GET['form_id'])) {
							  ...
							  if(isset($_POST['btn_save_wpdevart_form'])) {
								  ...
								  $update_flag = 1;
								  // get the user submitted form name
								  ...
								  foreach ($_POST as $name => $value) {
									  ...
									  if($explode[0] == "label") {	   
										   ...
										   if(isset($explode[2])) {
												...
												// Parent field contains id according to front-end form position 
												...
												// Save the newly created parent id in parent field array to determine its child fields
												$main_field_arr[$explode[1]] = $form_field_id;
										   }
										} // Checking for close label
										
										if($explode[0] == "sublabel")  { 
											// Checking the field type of the subfield 
											if(!empty($explode[2])) {
											...
											if($fieldtype == 'radio') {
												if(!empty($explode[2])) {
													// checking if current  radio button was selected as default 
											...
											if($fieldtype == 'options_list') {
												//	Checking if current  option  was selected as default 
												if( !empty($explode[2]) ) {
											...
											if($fieldtype == 'checkbox') {
												//	Checking if current  checkbox  was selected as default
												if(!empty($explode[2])) {
											...
											// If database id of the sub-label is set, update the record else create record
											if(!empty($explode[2])) {	
									   ...
									   //	parent child relationship because cancel button with have a subfield of redirection URL 
									   if($name == "cancel") {
							  ...
							  ?>
								<div id="addNewForm" class="generalForm">
									...
									<?php
									 ...
									 //getting the form submit button and advanced options
