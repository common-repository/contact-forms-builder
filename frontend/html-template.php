<?php
/**
 * wpdevart_template() function is used to create/display the front end form 
 * based on the shortcode and attributes (id) we provide e.g short code 
...
function wpdevart_template($atts) {
	...
	if(isset($atts['id']) && $atts['id']!= '') {
		...
		if($form_id == $confirm_form_id) 
		{
			...
			//	By default don't display contact form name on front end 
			$show_form_name = "no";
				...
				if( ($status == 'private' && (!isset($atts['role']) || empty($atts['role']))) ) {
					...
					if(empty($error_msg)) {
						...
						foreach ($form_fields as $key => $form_field) {
							...
							if( !empty($field_subfields) ) {	
								if($form_field->fieldtype == "radio" || $form_field->fieldtype == "checkbox" || $form_field->fieldtype == "options_list") {
									...
									<?php 
									// Loop through the subfields to show them on the page
					  ...
					  ?>
					   ...
					   // Checking if user has enabled the cancel button or not
